...
void UpdateStatusBar(ClockTicks arg)
{
    ...
    if (gViewSize == 2 || gViewSize == 3)
    {
        ...
        if (bKeyHolder)
        {
            static int nOffsetX = tilesiz[2201].x;
            static int nOffsetY = tilesiz[kSBarKeyHolderAlt].y>>1;
            DrawStatSprite(kSBarKeyHolderAlt, nOffsetX, 187-nOffsetY, 16, nPalette, 256|RS_TOPLEFT);
        }
